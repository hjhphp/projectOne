...
<div id="content">
	<div id="content-header">
		...
		<div class="btn-group">
			<a class="btn btn-large" title="友情链接列表"><i class="fa fa-tasks"></i></a>
		</div>
	</div>
	<div id="breadcrumb">
			...
			<div class="col-xs-12">
				<div class="widget-box">
					<div class="widget-title">
						...
						<?php else: ?>
						<h5>修改友情链接</h5>
						<input id="id" value="<?php echo ($info["l_id"]); ?>" type="hidden"/><?php endif; ?>
					</div>
						...
						<form  method="post" class="form-horizontal">							
                            ...
                            <div class="form-group">
                                ...
                                <div class=" col-sm-9 col-md-9 col-lg-5">
                                	<div class="input-group input-group-sm">
										<span class="input-group-addon"><i class="fa fa-globe"></i></span>
                                    	<input type="text" <?php if(empty($info['l_id'])): ?>value="<?php echo get_client_ip();?>"<?php else: ?>value="<?php echo ($info["l_ip"]); ?>"<?php endif; ?> class="form-control input-sm" />
                                    </div>
                                </div>
                                <div class=" col-sm-9 col-md-9 col-lg-5">
                                	<div class="input-group input-group-sm">
										<span class="input-group-addon"><i class="fa fa-globe"></i></span>
                                    	<input type="text" <?php if(empty($info['l_id'])): ?>value="<?php echo getIp(get_client_ip());?>"<?php else: ?>value="<?php echo (getIp($info["l_ip"])); ?>"<?php endif; ?> class="form-control input-sm" />
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class=" col-sm-9 col-md-9 col-lg-10">
                                	<div class="input-group input-group-sm">
                                    	...
                                    	<input type="text" <?php if(empty($info['l_id'])): ?>value="100"<?php else: ?>value="<?php echo ($info["l_sort"]); ?>"<?php endif; ?> class="form-control input-sm" placeholder="100" id="sort" />
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
								...
								<div class="col-sm-9 col-md-9 col-lg-10">
									<label><input type="radio" name="show" value="0" <?php if(($info['l_view']) == "0"): ?>checked<?php endif; ?>/> 不显示</label>
									<label><input type="radio" name="show" value="2" <?php if(($info['l_view']) == "2"): ?>checked<?php endif; ?>/> 首页</label>
									<label><input type="radio" name="show" value="1" <?php if(($info['l_view']) == "1"): ?>checked<?php endif; ?>/> 内页</label>
								</div>
							</div>
                            <?php if(!empty($info['l_id'])): ?><div class="form-group">
                                ...
                                <div class=" col-sm-9 col-md-9 col-lg-10">
                                	<div class="input-group input-group-sm">
										<span class="input-group-addon"><i class="fa fa-user"></i></span>
                                    	<input type="text" value="<?php echo (session('uname')); ?>" class="form-control input-sm" id="rname"  />
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class=" col-sm-9 col-md-9 col-lg-10">
                                	<div class="input-group input-group-sm">
                                    	...
                                    	<input type="text" value="<?php echo date("Y-m-d H:i:s",time());?>" class="form-control input-sm" id="rtime" />
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
								...
								<div class="col-sm-9 col-md-9 col-lg-10">
									<textarea rows="5" class="form-control" id="rcontent" id="rcontent"><?php echo ($info["l_rcontent"]); ?></textarea>
								</div>
							</div>
							<div class="form-group">
...
<script>
    ...
    $('#addL').click(function(){
		...
		$.ajax({
			...
			async:true,
			data:{"l_name":name,"l_email":email,"l_url":url,"l_content":remark,"l_sort":sort,"l_view":show},
			success:function(data){
				if(data.error==0){
					succ("添加链接完成!",data.msg,"/Admin/Link/linkList");
				}else{
					err(data.msg);
    ...
    $('#editL').click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.error==0){
					succ("处理链接完成!",data.msg,"/Admin/Link/linkList");
				}else{
					err(data.msg);
