				...
				<div class="widget-content">
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($v["l_id"]); ?></td>
								<td><?php echo ($v["l_name"]); ?></td>
								<td><?php echo (msubstr($v["l_content"],0,80,'utf-8',true)); ?></td>
								<td>
									<?php switch($v['l_view']): case "0": ?><span class=" btn-dark-red btn-xs">不显示</span><?php break;?>
										<?php case "1": ?><span class=" btn-dark-green btn-xs">内页显示</span><?php break;?>
										<?php case "2": ?><span class=" btn-dark-green btn-xs">首页显示</span><?php break; endswitch;?>
								</td>
								<td><a class="btn btn-success btn-sm" href="<?php echo U('/Admin/Link/linkEdit',array('id'=>$v['l_id']));?>">编辑</a>&nbsp;&nbsp;
	...
	$(".delL",this).click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.error==0){
					succ("完成!",data.msg,"/index.php/Admin/Link/linkList.html");
				}else{
					err("错误!",data.msg);
