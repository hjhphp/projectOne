					...
					<table class="table table-bordered table-striped table-hover">
						...
						<tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($k["u_id"]); ?></td>
								<td><?php echo ($k["u_name"]); ?></td>
								<td>
									<?php switch($k['u_class']): case "1": ?>最高管理员<?php break;?>
										<?php case "2": ?>普通管理员<?php break;?>
										<?php default: ?>普通用户<?php endswitch;?>
								</td>
								<td><?php echo ($k["u_email"]); ?></td>
								<td><?php echo ($k["u_time"]); ?></td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('/Admin/User/userEdit',array('id'=>$k['u_id']));?>">编辑</a>&nbsp;&nbsp;
    	...
    	$(".deluser",this).click(function(){
    		...
    		$.ajax({
    			...
    			success:function(data){
    				if(data.error==0){
    					succ("完成!",data.msg,"/index.php/Admin/User/userList.html");
    				}else{
    					err("错误!",data.msg);
