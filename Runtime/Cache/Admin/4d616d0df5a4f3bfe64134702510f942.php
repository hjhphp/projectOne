                ...
                <div class="widget-content">
                    <table class="table table-bordered table-striped table-hover ">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><tr>
                                <td class="id"><?php echo ($k["top_id"]); ?></td>
                                <td><?php echo ($k["top_name"]); ?></td>
                                <td><?php echo (msubstr($k["top_content"],0,120,'utf-8',true)); ?></td>
                                <td><?php echo ($k["createtime"]); ?></td>
                                <td>
                                    <?php switch($k['is_show']): case "0": ?><span class=" btn-dark-red btn-xs">不显示</span><?php break;?>
                                        <?php case "1": ?><span class=" btn-dark-green btn-xs">显示</span><?php break;?>
                                        <?php default: ?>暂无数据<?php endswitch;?>
                                </td>
                                <td>
                                    <a class="btn btn-success btn-sm" href="<?php echo U('/Admin/Topline/topEdit',array('id'=>$k['top_id']));?>">编辑</a>&nbsp;&nbsp;
        ...
        $(".deltag",this).click(function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.error==0){
                        succ("完成!",data.msg,"/index.php/Admin/Topline/topList.html");
                    }else{
                        err("错误!",data.msg);
